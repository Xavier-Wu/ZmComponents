  ...
  watch: {
    ...
    options() {
      this.$emit(
        'change-data',
        this.value1,
        this.options.find(e => e[this.valueKey] === this.value1)
      )
    }
  },
  async created() {
  ...
  methods: {
    async fetchOptions() {
      return new Promise(async (resolve, reject) => {
        if (this.dictKey) {
          this.options = await this.$request.get('/v1/common/getLabelList', {
            params: { type: this.dictKey }
          })
          ...
          this.options = await this.$request.get(this.action, {
            params: params
          })
          this.$emit('after-action', this.options)
        }
